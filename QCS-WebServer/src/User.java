...
public class User {
    //    注册
    public static boolean regist(String userid, String username, String userpwd) {
        System.out.printf(userid, username, userpwd);
        String sql = "insert into `userinfo`(userid,username,userpwd) value(?,?,?)";
    ...
    public static String selectpwd(String userid, String userpwd) {
        String flag = "false";
        boolean p = false;
        String sql = "select * from `userinfo` where userid=?&&password=?";
        DBUtil dbUtil = new DBUtil(sql);
        try {
            dbUtil.preparedStatement.setString(1, userid);
